...
const NavbarDisplay: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLogos = async () => {
      try {
        const darkLogoResponse = await axios.get('https://queb.online/api/me/2/?format=json');
        const lightLogoResponse = await axios.get('https://queb.online/api/me/1/?format=json');
