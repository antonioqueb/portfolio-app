...
const Home: React.FC = () => {
  ...
  return (
    <div>
      <NavbarDisplay />
      <h1 className={isDarkMode ? "text-light" : "text-dark"} >{title}</h1>
      <p className={isDarkMode ? "text-light" : "text-dark"}>{date}</p>
      <p className={isDarkMode ? "text-light" : "text-dark"}>{content}</p>
      <ul>
        {tags.map((tag) => (
          <li key={tag} className={isDarkMode ? "text-light" : "text-dark"} >{tag}</li>
        ))}
      </ul>
      <FooterDisplay />
    </div>
